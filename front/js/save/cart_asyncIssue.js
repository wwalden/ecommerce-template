...
//  for (i=0;i<=count-1;i++) {
    let reference = Object.values(Object.values(myJson)[0]);    // renvoie un Array avec la liste des quantités (une entrée par couleur) (pour Qty)
    let myIdd = Object.keys(myJson)[0];                         // trouve l'ID (pour prix)  
    console.log(getPrice(myIdd));
    //let pcebyRef = getPrice(myIdd); // NOT WORKING !!
    //alert(pcebyRef);
    //alert(getPrice(Object.keys(myJson)[i]));
    ...
    myTotPce += pceByRef * countByRef                           // donne le prix pour chaque ID du panier (pour prix)                                                        
  //}
...
/*
// PUIS FETCH POUR RÉCUP LE PRIX: NE MARCHE PAS
let myData = localStorage.getItem("carty");
let myJson = JSON.parse(myData);
let count = Object.keys(myJson).length;

for (i=0; i<=count-1; i++) {
  let prodId = Object.keys(myJson)[i];
...
function getPrice() {
  ...
  .then(function(value) {
    let prodPrice = parseInt(value.price);
    updateTotal(prodPrice);
  });
}
}
*/
